...
namespace PR4_O.Pages
{
    public partial class Task1Page : Page
    {
        ...
        private void Calculate_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!Parse(TbT, "t", out double t)) return;
            if (!Parse(TbG, "g", out double g)) return;

            double inner = y + Math.Cbrt(x - 1);
            if (inner < 0) { Warn("Выражение под корнем 4-й степени отрицательно."); return; }

            double den = Math.Abs(x - y) * (Math.Sin(z) * Math.Sin(z) + t * g * z);
        ...
        {
            TbX.Text = TbY.Text = TbZ.Text = TbT.Text = TbG.Text = TbResult.Text = "";
        }
    }
}
